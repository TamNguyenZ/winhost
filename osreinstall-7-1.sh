...
download_image() {
    ...
    for attempt in $(seq 1 $retries); do
        ...
        if command -v aria2c &>/dev/null; then
            ...
            # If a previous attempt left a stale/corrupt .aria2 control file,
            # aria2c can refuse to resume and fail immediately. On the first
            # attempt, clear any leftover control file so we start clean.
            if [[ "$attempt" -eq 1 && -f "${dest}.aria2" ]]; then
                log_info "Removing stale aria2 control file from a previous run..."
                rm -f "${dest}.aria2"
            fi

            # aria2c with: 16 connections, continue/resume, show progress
            # --file-allocation=none: avoids fallocate() failing/hanging on
            #   overlayfs/tmpfs/NFS (common in rootless containers/JupyterHub),
            #   which otherwise makes aria2c appear to "not work" at all.
            # --check-certificate=false: tolerate self-signed/expired certs,
            #   matching the -k/--no-check-certificate used by curl/wget below.
            # --allow-overwrite=true: force overwrite instead of silently
            #   refusing when the destination already exists.
...
build_grub_entry() {
    ...
    # Build GRUB search directive — prefer file-based (no UUID dependency)
    local search_directive=""
    local boot_uuid=""

    # Try to get UUID as fallback info, but PRIMARY method is --file search
    ...
    # PRIMARY: search for our known kernel file (most reliable method)
    # This works regardless of UUID, partition layout, etc.
    search_directive="search --no-floppy --file --set=root /osreinstall-vmlinuz"

    # If --file search fails, try UUID as fallback
    if [[ -n "$boot_uuid" ]]; then
