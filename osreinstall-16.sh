...
inspect_image() {
    ...
    # kpartx is the RELIABLE way to access partitions inside raw images
    # (losetup --partscan is unreliable on some kernels)
    if ! command -v kpartx &>/dev/null; then
        log_info "  Installing kpartx..."
        apt-get update -qq 2>/dev/null || true
        apt-get install -y -qq kpartx 2>/dev/null || true
    fi
    ...
    if command -v kpartx &>/dev/null; then
        ...
        local kpartx_out
        kpartx_out=$(kpartx -av "$img" 2>&1)
        log_info "  kpartx mappings:" | tee -a "$IMAGE_INSPECT_LOG"
        echo "$kpartx_out" >> "$IMAGE_INSPECT_LOG"

        # Get the loop device kpartx created
        loopdev=$(echo "$kpartx_out" | grep -oP 'add map \K[^ ]+' | head -1)
        if [[ -z "$loopdev" ]]; then
            loopdev=$(echo "$kpartx_out" | grep -oP 'loop[0-9]+' | head -1)
            [[ -n "$loopdev" ]] && loopdev="/dev/mapper/$loopdev"
        fi
        [[ -z "$loopdev" ]] && loopdev=$(losetup --find --show "$img" 2>/dev/null)
    else
        # Fallback to losetup
    ...
    if [[ -z "$loopdev" ]]; then
        log_warn "  Cannot mount image (unsupported format)"
        return 1
    fi
    sleep 1

    # ----- 3. Scan ALL partitions (kpartx mapper + loop) -----
    log_info "  Partitions:" | tee -a "$IMAGE_INSPECT_LOG"
    local parts_found=0
    local mounted_ok=false

    # Build partition list from kpartx (most reliable — always creates all partitions)
    local part_list
    part_list=$(ls /dev/mapper/ 2>/dev/null | grep -E 'loop[0-9]+p[0-9]+' | sort -V | sed 's|^|/dev/mapper/|')

    # Also check direct loop devices
    local loop_base
    loop_base=$(losetup -j "$img" 2>/dev/null | cut -d: -f1 | head -1 | xargs basename 2>/dev/null)
    [[ -z "$loop_base" ]] && loop_base=$(basename "$loopdev" 2>/dev/null)

    for pnum in 1 2 3 4 5 6; do
        for dev in "/dev/${loop_base}p${pnum}" "/dev/${loop_base}${pnum}" "/dev/mapper/${loop_base}p${pnum}"; do
            [[ -b "$dev" ]] && part_list="${part_list}${part_list:+$'\n'}$dev"
        done
    done

    log_info "  DEBUG: partition devices found: $(echo "$part_list" | wc -l)" | tee -a "$IMAGE_INSPECT_LOG"
    echo "$part_list" >> "$IMAGE_INSPECT_LOG"

    if [[ -z "$part_list" ]]; then
        log_warn "  No partition devices — cannot inspect filesystem internals"
        return 1
    fi
    ...
    if $mounted_ok && [[ -d "$mountdir" ]]; then
        ...
        if [[ -f "$mountdir/Windows/System32/ntoskrnl.exe" ]] || \
           [[ -f "$mountdir/Windows/System32/winload.efi" ]]; then
            ...
            # Windows version (fast check via license file)
            local win_ver=""
            if [[ -f "$mountdir/Windows/System32/license.rtf" ]]; then
                win_ver=$(head -c 500 "$mountdir/Windows/System32/license.rtf" 2>/dev/null | grep -oE '(Windows [A-Za-z]+ [0-9]+)' | head -1 || echo "")
            fi
            [[ -z "$win_ver" ]] && win_ver="Windows (detected)"
            log_info "  Version: $win_ver" | tee -a "$IMAGE_INSPECT_LOG"

            # ----- Windows: check virtio drivers -----
            log_info "  Driver check:" | tee -a "$IMAGE_INSPECT_LOG"

            # Storage drivers (use ls + wildcard, much faster than find)
            local drv_dir
            for drv_dir in "Windows/System32/drivers" "Windows/System32/DriverStore/FileRepository"; do
                if ls "$mountdir/$drv_dir/viostor.sys" 2>/dev/null || \
                   ls "$mountdir/$drv_dir/vioscsi.sys" 2>/dev/null || \
                   ls "$mountdir/$drv_dir/storvsc.sys" 2>/dev/null || \
                   ls "$mountdir/$drv_dir/pvscsi.sys" 2>/dev/null; then
                    IMAGE_HAS_VIRTIO_STOR=true
                    log_info "    [OK] Storage: virtio/SCSI driver present" | tee -a "$IMAGE_INSPECT_LOG"
                    break
                fi
                # DriverStore: check one level deep
                if ls "$mountdir/$drv_dir"/viostor*/*.sys 2>/dev/null | grep -q .; then
                    IMAGE_HAS_VIRTIO_STOR=true
                    log_info "    [OK] Storage: virtio driver found in DriverStore" | tee -a "$IMAGE_INSPECT_LOG"
                    break
                fi
            done
            ...
            # Network drivers
            for drv_dir in "Windows/System32/drivers" "Windows/System32/DriverStore/FileRepository"; do
                if ls "$mountdir/$drv_dir/netkvm.sys" 2>/dev/null || \
                   ls "$mountdir/$drv_dir/netvsc.sys" 2>/dev/null || \
                   ls "$mountdir/$drv_dir/vmxnet3ndis6.sys" 2>/dev/null; then
                    IMAGE_HAS_VIRTIO_NET=true
                    log_info "    [OK] Network: virtio/vmxnet3 driver present" | tee -a "$IMAGE_INSPECT_LOG"
                    break
                fi
                if ls "$mountdir/$drv_dir"/netkvm*/*.sys 2>/dev/null | grep -q .; then
                    IMAGE_HAS_VIRTIO_NET=true
                    log_info "    [OK] Network: virtio driver found in DriverStore" | tee -a "$IMAGE_INSPECT_LOG"
                    break
                fi
            done

            # NVMe driver
            if ls "$mountdir/Windows/System32/drivers/stornvme.sys" 2>/dev/null; then
                IMAGE_HAS_NVME=true
                log_info "    [OK] NVMe: stornvme.sys" | tee -a "$IMAGE_INSPECT_LOG"
            fi

            # ----- Windows: RDP check -----
            # Quick check: Terminal Services DLL exists
            if ls "$mountdir/Windows/System32/termsrv.dll" 2>/dev/null; then
                IMAGE_HAS_RDP=true
