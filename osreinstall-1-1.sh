...
build_stage2_initramfs() {
    ...
    # NOTE: do NOT symlink $tmpdir/init to /bin/sh here — that would make
    # this file a symlink to the HOST's real /bin/sh, and the `cat >`
    # below would then follow the symlink and try to truncate/overwrite
    # the host's live /bin/sh binary, which is always in use -> ETXTBSY.
    rm -f "$tmpdir/init"
