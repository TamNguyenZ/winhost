...
if [ -f /etc/os-release ]; then
    . /etc/os-release
    # Some Debian/Ubuntu derivatives (Linux Mint, Pop!_OS, Zorin OS,
    # elementary OS, MX Linux, ...) set ID to their own name but declare
    # ID_LIKE="ubuntu debian" (or similar). Checking ID alone rejects them
    # even though they are Ubuntu/Debian underneath. Fall back to ID_LIKE.
    case "$ID" in
        debian|ubuntu) : ;;
        *)
            case " ${ID_LIKE:-} " in
...
install_aria2() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        dnf install -y -q aria2 2>/dev/null
    elif command -v yum >/dev/null 2>&1; then
        yum install -y -q aria2 2>/dev/null
    fi
    command -v aria2c >/dev/null 2>&1 && return 0

    # Fallback: package manager missing/offline — fetch a static aria2c
    # binary directly so the fast multi-connection path still works.
    log_warn "Package manager could not install aria2 — trying static binary fallback..."
    local tmp_tgz="/tmp/aria2-static.tar.gz" extract_dir="/tmp/aria2-static-extract"
    rm -rf "$tmp_tgz" "$extract_dir"; mkdir -p "$extract_dir"
    local aria2_url="https://github.com/q3aql/aria2-static-builds/releases/latest/download/aria2-x86_64-linux-musl_static.tar.gz"
    if wget --no-check-certificate -4 -qO "$tmp_tgz" "$aria2_url" 2>/dev/null \
        || curl -LfsSo "$tmp_tgz" "$aria2_url" 2>/dev/null; then
        tar -xzf "$tmp_tgz" -C "$extract_dir" 2>/dev/null || true
        local found_bin
        found_bin="$(find "$extract_dir" -type f -name 'aria2c' 2>/dev/null | head -1)"
        if [ -n "$found_bin" ]; then
            install -m 0755 "$found_bin" /usr/local/bin/aria2c 2>/dev/null || cp "$found_bin" /usr/local/bin/aria2c
            chmod +x /usr/local/bin/aria2c
            export PATH="/usr/local/bin:$PATH"
        fi
    fi
    rm -rf "$tmp_tgz" "$extract_dir"
...
if ! $has_virtio; then
    ...
    install_aria2 || true
    kernel_dl_ok=false
    if command -v aria2c >/dev/null 2>&1 && aria2c -x 8 -s 8 -c \
        -d "$(dirname "$deb_file")" -o "$(basename "$deb_file")" "$auto_kernel_url" 2>&1 | tee -a "$LOGFILE"; then
        kernel_dl_ok=true
    elif wget --no-check-certificate -4 -q --show-progress -O "$deb_file" "$auto_kernel_url" 2>&1 | tee -a "$LOGFILE"; then
        kernel_dl_ok=true
    fi
    if $kernel_dl_ok && [ -s "$deb_file" ]; then
...
# -----------------------------------------------------------------------
# Load kernel modules — STORAGE drivers FIRST, then NETWORK.
#
# The original code only loaded network drivers (for debug SSH). But
# modprobe'ing storage drivers is essential when:
#   - Using a downloaded generic kernel (drivers are modules, not built-in)
#   - The VPS disk controller differs from the image-build host
# Without virtio_blk/ahci/nvme/xen-blkfront loaded, the kernel cannot see
# the target disk → cannot mount the image partition → cannot dd → fail.
#
# virtio_pci must come BEFORE virtio_blk/virtio_scsi (PCI bus driver first).
# -----------------------------------------------------------------------
log_msg "=== LOADING STORAGE DRIVERS ==="
for mod in virtio_pci virtio_blk virtio_scsi virtio_console \
           vmw_pvscsi vmw_vmxnet3 \
           hv_storvsc hv_vmbus hv_utils \
           xen-blkfront xen_scsifront \
           nvme nvme-core nvme-fabrics \
           ahci libata ata_piix pata_acpi \
           sd_mod sr_mod sg scsi_mod scsi_common; do
    modprobe "$mod" 2>/dev/null && log_msg "Loaded storage: $mod" || true
done

# Trigger device node creation for newly loaded storage drivers.
# mdev -s scans /sys and creates /dev nodes; SCSI host rescan discovers
# disks that appeared after the driver was loaded.
/sbin/mdev -s 2>/dev/null || true
for host in /sys/class/scsi_host/*/scan; do
    [ -e "$host" ] && echo "- - -" > "$host" 2>/dev/null || true
done
# Also rescan NVMe subsystem
for ctrl in /sys/class/nvme/nvme*/rescan; do
    [ -e "$ctrl" ] && echo 1 > "$ctrl" 2>/dev/null || true
done
sleep 2
/sbin/mdev -s 2>/dev/null || true

# Wait for block devices to appear (timeout 10s)
wait_count=0
while [ -z "$(ls /sys/block/ 2>/dev/null | grep -vE 'loop|ram|sr')" ] && [ "$wait_count" -lt 10 ]; do
    sleep 1
    wait_count=$((wait_count + 1))
    /sbin/mdev -s 2>/dev/null || true
done
log_msg "Block devices visible: $(ls /sys/block/ 2>/dev/null | grep -vE 'loop|ram|sr' | tr '\n' ' ')"

# THEN load network drivers (for debugging via SSH/VNC if needed)
log_msg "=== LOADING NETWORK DRIVERS ==="
for mod in virtio_net virtio_pci vmxnet3 hv_netvsc xen_netfront e1000 e1000e r8169 igb ixgbe; do
    modprobe "$mod" 2>/dev/null && log_msg "Loaded network: $mod" || true
done
sleep 2
NIC_UP=false
...
STAGE2INIT
    ...
    for tool in dd lsblk blkid findfs mount umount chroot sync blockdev sha256sum fdisk sfdisk efibootmgr sgdisk mkfs.vfat mkfs.fat gunzip modprobe insmod lsmod depmod kmod; do
    ...
    # -----------------------------------------------------------------------
    # Bundle kernel modules for STORAGE + NETWORK drivers into the initramfs.
    #
    # This is CRITICAL for cross-hypervisor portability. When WinBoxes uses
    # a downloaded generic Debian kernel (whose drivers are modules, not
    # built-in), modprobe needs the actual .ko files + modules.dep/alias
    # to resolve PCI IDs to driver modules. Without this, the Stage-2 init
    # cannot see the target disk (no virtio_blk/ahci/nvme loaded) → cannot
    # mount the image partition → cannot dd → install fails silently.
    #
    # Strategy: selectively copy storage + network module subtrees (not the
    # full 300MB tree), plus modules.builtin so modprobe knows which drivers
    # are already compiled into the kernel and skips them.
    # -----------------------------------------------------------------------
    if [ -n "$KERNEL_VER" ] && [ -d "/lib/modules/$KERNEL_VER" ]; then
        log_info "Bundling kernel modules ($KERNEL_VER) for storage + network drivers..."
        MOD_SRC="/lib/modules/$KERNEL_VER"
        MOD_DEST="$tmpdir/lib/modules/$KERNEL_VER"
        mkdir -p "$MOD_DEST"

        # Module metadata — modprobe REQUIRES these to resolve aliases.
        # modules.builtin tells modprobe which drivers are already in the
        # kernel (built-in = B), so it skips them instead of erroring.
        for f in modules.dep modules.alias modules.builtin modules.builtin.alias \
                 modules.order modules.builtin.modinfo modules.symbols \
                 modules.alias.bin modules.dep.bin modules.symbols.bin; do
            cp "$MOD_SRC/$f" "$MOD_DEST/" 2>/dev/null || true
        done

        # Selectively copy storage + network driver module subtrees.
        # This covers all major hypervisor disk/NIC drivers:
        #   virtio (KVM/QEMU)  | vmw_pvscsi (VMware) | hv_storvsc (Hyper-V)
        #   xen-blkfront (Xen) | nvme (modern)       | ahci/libata (SATA)
        for subtree in \
            ...
            kernel/drivers/pci \
            kernel/drivers/acpi \
            kernel/drivers/cdrom; do
            if [ -e "$MOD_SRC/$subtree" ]; then
                mkdir -p "$MOD_DEST/$(dirname "$subtree")"
                cp -a "$MOD_SRC/$subtree" "$MOD_DEST/$subtree" 2>/dev/null || true
            fi
        done

        # Also copy the kmod shared lib if modprobe is the dynamic kmod version
        if command -v modprobe >/dev/null 2>&1; then
            for lib in $(ldd "$(command -v modprobe)" 2>/dev/null | awk '/=>/ {print $3}'); do
                if [ -f "$lib" ] && [ ! -f "$tmpdir/$lib" ]; then
                    mkdir -p "$tmpdir/$(dirname "$lib")"
                    cp -L "$lib" "$tmpdir/$lib" 2>/dev/null || true
                fi
            done
        ...
        local mod_count
        mod_count=$(find "$MOD_DEST" -name '*.ko*' 2>/dev/null | wc -l)
        log_info "Bundled $mod_count kernel modules for storage + network"
    else
        log_warn "No /lib/modules/$KERNEL_VER found — storage drivers must be built-in to the kernel"
    fi
    ...
    # -----------------------------------------------------------------------
    # CRITICAL: verify the busybox binary INSIDE the archive is byte-identical
    # to the source we copied from $BUSYBOX_BIN.
    #
    # ROOT CAUSE this catches: if /tmp fills up mid-pipeline (find | cpio |
    # gzip), gzip can be silently truncated/short-written while still
    # exiting 0 in a pipeline (its own exit status isn't checked here — only
    # cpio's is, via PIPESTATUS not being read). A truncated busybox inside
    # the initramfs still passes the "file exists" check above, but the
    # kernel's exec() of a corrupt ELF segfaults immediately — which is
    # exactly the "Attempted to kill init! exit code=0x00000100" panic with
    # a userspace RIP fault (handle_mm_fault / exc_page_fault) seen in the
    # field, since PID 1 cannot be allowed to die.
    # -----------------------------------------------------------------------
    verify_tmpdir="$(mktemp -d /tmp/winboxes-verify.XXXXXX)"
    if zcat "$STAGE2_INITRD" 2>/dev/null | (cd "$verify_tmpdir" && cpio -idm --quiet 2>/dev/null); then
        if [ -f "$verify_tmpdir/bin/busybox" ]; then
            SRC_SUM="$(sha256sum "$BUSYBOX_BIN" 2>/dev/null | awk '{print $1}')"
            PACKED_SUM="$(sha256sum "$verify_tmpdir/bin/busybox" 2>/dev/null | awk '{print $1}')"
            if [ -n "$SRC_SUM" ] && [ "$SRC_SUM" = "$PACKED_SUM" ]; then
                log_success "busybox integrity verified (sha256 matches source, no truncation)"
            else
                rm -rf "$verify_tmpdir"
                die "FATAL: busybox inside the built initramfs does not match the source binary (sha256 mismatch). This usually means /tmp ran out of space during archive creation, producing a truncated image that would segfault at boot. Free up space in /tmp and re-run."
            fi
        else
            rm -rf "$verify_tmpdir"
            die "FATAL: could not extract /bin/busybox back out of the built archive for verification."
        fi
    else
        log_warn "Could not re-extract archive for integrity verification (cpio -idm unavailable) — skipping deep check."
    fi
    rm -rf "$verify_tmpdir"
...
menuentry "${GRUB_MENU_TITLE}" --id ${GRUB_MENU_ID} {
    # NOTE: some hosting-provider GRUB builds ship a core.img whose \$prefix
    # does not contain every i386-pc module file (e.g. no ext4.mod), which
    # previously printed a scary but harmless
    #   "error: file '/grub/i386-pc/ext4.mod' not found"
    # right before the menu continued to boot anyway. We now check the
    # module file actually exists under \$prefix before calling insmod, so
    # the error simply never occurs instead of being printed and ignored.
    function safe_insmod {
        if [ -f "\$prefix/i386-pc/\$1.mod" ]; then
            insmod "\$1"
        elif [ -f "\$prefix/\$1.mod" ]; then
            insmod "\$1"
        fi
    }
    safe_insmod part_msdos
    safe_insmod part_gpt
    safe_insmod ext2
    safe_insmod ext4
    safe_insmod xfs
    safe_insmod btrfs
    ...
    safe_insmod normal
    safe_insmod linux
$(echo "$firmware_modules" | sed 's/^insmod /    safe_insmod /')
