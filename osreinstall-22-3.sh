...
build_stage2_initramfs() {
    ...
    # Copy busybox — STRONGLY prefer a statically-linked build. A dynamic
    # busybox needs its ELF interpreter (ld-linux-*.so) present inside the
    # initramfs to execute at all; missing it causes /init (which execs
    # busybox via its shebang) to fail, and the kernel panics with
    # "No working init found" after every fallback init path also fails.
    find_static_busybox() {
        local candidates=(
        ...
        # Also check whatever's on PATH
        if command -v busybox &>/dev/null; then
            candidates+=("$(command -v busybox)")
        fi
        for c in "${candidates[@]}"; do
            if [[ -x "$c" ]] && file "$c" 2>/dev/null | grep -qi "statically linked"; then
                echo "$c"
                return 0
            fi
        done
    ...
    if BUSYBOX_BIN=$(find_static_busybox); then
        log_info "Using static busybox: $BUSYBOX_BIN"
    else
        log_warn "No static busybox found — installing busybox-static..."
        apt-get install -y -qq busybox-static 2>/dev/null || true
        if BUSYBOX_BIN=$(find_static_busybox); then
            log_info "Using static busybox: $BUSYBOX_BIN"
        elif command -v busybox &>/dev/null; then
            BUSYBOX_BIN="$(command -v busybox)"
            log_warn "Only a DYNAMIC busybox is available ($BUSYBOX_BIN). Will attempt to bundle its shared libraries + interpreter, but this is fragile — installing busybox-static on the host is strongly recommended."
        else
            die "Cannot find busybox (static or dynamic). Required for Stage 2 initramfs."
        fi
    fi
    cp -L "$BUSYBOX_BIN" "$tmpdir/bin/busybox"
...
STAGE2INIT
    ...
    # Copy required libraries + ELF interpreter for the tools we added
    if command -v ldd &>/dev/null; then
        local scan_bins=()
        for b in dd zstd gunzip xz bzip2 qemu-img sha256sum busybox; do
            [[ -x "$tmpdir/bin/$b" ]] && scan_bins+=("$tmpdir/bin/$b")
        done

        local libs=""
        if [[ ${#scan_bins[@]} -gt 0 ]]; then
            # Resolved shared libs: lines like "libc.so.6 => /path (0xaddr)"
            libs=$(ldd "${scan_bins[@]}" 2>/dev/null | awk '/=>/ {print $3}' | sort -u || true)
            # The ELF interpreter itself: lines like "/lib64/ld-linux-x86-64.so.2 (0xaddr)"
            # — only 2 fields (path, address), no "=>". This is REQUIRED for any
            # dynamically-linked binary to run at all.
            local interp
