...
show_usage() {
    cat <<EOF
Usage: $0 --image-url <URL> [--disk <DEVICE>] [--dry-run]
...
Examples:
  ...
  $0 --image-url https://example.com/ubuntu24.img.gz --disk /dev/vda
EOF
    exit 0
}
...
# 1c. Reject container environments
if grep -qE '(docker|lxc|containerd|kubepods)' /proc/1/cgroup 2>/dev/null; then
    die "Running in container environment. This tool requires a real OS or VM."
fi
if [[ -f /.dockerenv ]]; then
    die "Running inside Docker container. This tool requires a real OS or VM."
fi
if command -v systemd-detect-virt &>/dev/null; then
    VIRT_TYPE=$(systemd-detect-virt -c 2>/dev/null || echo "none")
...
# Get IP addresses
if [[ -n "$IFACE" ]]; then
    IPV4=$(ip -4 addr show "$IFACE" 2>/dev/null | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -1 || echo "none")
    IPV6=$(ip -6 addr show "$IFACE" 2>/dev/null | grep -oP '(?<=inet6\s)[0-9a-f:]+' | head -1 || echo "none")
...
download_image() {
    ...
    for attempt in $(seq 1 $retries); do
        ...
        if wget --no-check-certificate -4 --progress=bar:force:noscroll \
            -O "$dest" "$url" 2>&1 | tee -a "$LOGFILE"; then
            log_success "Download completed"
            return 0
        fi

        if curl -L --progress-bar -o "$dest" "$url" 2>&1 | tee -a "$LOGFILE"; then
            log_success "Download completed (curl fallback)"
            return 0
        fi
...
if [[ -f "$STAGE2_IMAGE" ]]; then
    ...
    if (( EXISTING_SIZE > 1048576 )); then
        log_warn "Image already exists at $STAGE2_IMAGE ($((EXISTING_SIZE/1024/1024)) MB)"
        echo -n "Re-download? (y/N): "
        read -r REDOWNLOAD
        if [[ "$REDOWNLOAD" =~ ^[Yy] ]]; then
            rm -f "$STAGE2_IMAGE" "$STAGE2_IMAGE_CHECKSUM"
        else
            log_info "Using existing image file"
        fi
    fi
fi
...
if [[ ! -f "$STAGE2_IMAGE" ]]; then
    if $DRY_RUN; then
        ...
        sha256sum "$STAGE2_IMAGE" > "$STAGE2_IMAGE_CHECKSUM"
        log_info "SHA256: $(cut -d' ' -f1 "$STAGE2_IMAGE_CHECKSUM")"
    fi
fi
...
ROOT_PART=""
for part in ${TARGET_DISK}2 ${TARGET_DISK}1 ${TARGET_DISK}p2 ${TARGET_DISK}p1; do
    # For nvme: /dev/nvme0n1p2, /dev/nvme0n1p1
    # For others: /dev/sda2, /dev/sda1
    if [[ -b "$part" ]]; then
        ROOT_PART="$part"
        break
    fi
...
# For nvme devices with p-suffix
if [[ -z "$ROOT_PART" ]]; then
    for part in ${TARGET_DISK}p2 ${TARGET_DISK}p1; do
        if [[ -b "$part" ]]; then
            ROOT_PART="$part"
            break
        fi
...
if [[ -z "$ROOT_PART" ]]; then
    # Scan all partitions
    log_msg "Scanning all partitions to find root..."
    lsblk -o NAME,FSTYPE,SIZE,MOUNTPOINT "$TARGET_DISK" >> "$LOG" 2>&1
    ROOT_PART=$(lsblk -nlo NAME "${TARGET_DISK}" 2>/dev/null | tail -1)
    if [[ -n "$ROOT_PART" ]]; then
        ROOT_PART="/dev/$ROOT_PART"
    fi
fi

log_msg "Root partition: ${ROOT_PART:-not found}"
...
if [[ -n "$ROOT_PART" ]]; then
    ...
    if mountpoint -q /mnt 2>/dev/null; then
        ...
        # Try to regenerate initramfs inside the new OS (critical for generic drivers)
        log_msg "Regenerating initramfs for new OS..."
        if chroot /mnt update-initramfs -c -k all 2>/dev/null; then
            ...
            log_msg "WARNING: Could not regenerate initramfs. The new OS may fail to boot."
        fi
        ...
        if [[ -d /sys/firmware/efi ]]; then
            # UEFI
            chroot /mnt grub-install --target=x86_64-efi --efi-directory=/boot/efi --removable "$TARGET_DISK" 2>&1 | tee -a "$LOG" || {
...
STAGE2INIT
    ...
    # Copy required libraries for the tools we added
    local needed_libs
    needed_libs=$(ldd "$tmpdir/bin/dd" "$tmpdir/bin/zstd" "$tmpdir/bin/gunzip" 2>/dev/null | grep -oP '/\S+' | sort -u || true)
    ...
    # Also ensure standard lib directories
    if [[ -d /lib/x86_64-linux-gnu ]]; then
        cp -a /lib/x86_64-linux-gnu/libc.so* "$tmpdir/lib/x86_64-linux-gnu/" 2>/dev/null || true
        cp -a /lib/x86_64-linux-gnu/libz* "$tmpdir/lib/x86_64-linux-gnu/" 2>/dev/null || true
        cp -a /lib/x86_64-linux-gnu/liblzma* "$tmpdir/lib/x86_64-linux-gnu/" 2>/dev/null || true
        cp -a /lib/x86_64-linux-gnu/liblz4* "$tmpdir/lib/x86_64-linux-gnu/" 2>/dev/null || true
        cp -a /lib/x86_64-linux-gnu/libpthread* "$tmpdir/lib/x86_64-linux-gnu/" 2>/dev/null || true
    fi
...
menuentry "${GRUB_MENU_TITLE}" --id ${GRUB_MENU_ID} {
    ...
    # Search for the boot partition by label or filesystem
    search --no-floppy --fs-uuid --set=root $(blkid -s UUID -o value $(df --output=source /boot 2>/dev/null | tail -1) 2>/dev/null || echo "00000000-0000-0000-0000-000000000000")
