...
inspect_image() {
    ...
    for pnum in 1 2 3 4 5; do
        for part_dev in "${loopdev}p${pnum}" "${loopdev}${pnum}"; do
            [[ -b "$part_dev" ]] || continue
            local psize_bytes ptype plabel
            psize_bytes=$(blockdev --getsize64 "$part_dev" 2>/dev/null || echo 0)
            if (( psize_bytes > 1073741824 )); then
                psize=$(awk "BEGIN {printf \"%.1fG\", $psize_bytes/1073741824}")
            elif (( psize_bytes > 1048576 )); then
                psize=$(awk "BEGIN {printf \"%.0fM\", $psize_bytes/1048576}")
            else
                psize="${psize_bytes}"
            fi
            ...
            if [[ "$ptype" =~ ^(ext4|xfs|btrfs|ext3|ext2|ntfs|ntfs3)$ ]]; then
                local part_mnt="/tmp/osreinstall-p${pnum}"
                mkdir -p "$part_mnt"
