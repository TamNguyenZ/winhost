...
inspect_image() {
    ...
    for pnum in 1 2 3 4 5; do
        for part_dev in "${loopdev}p${pnum}" "${loopdev}${pnum}"; do
            [[ -b "$part_dev" ]] || continue
            local psize ptype plabel
            psize=$(blockdev --getsize64 "$part_dev" 2>/dev/null | awk '{printf "%.1fG", $1/1073741824}')
            ...
            if [[ "$ptype" == "vfat" ]]; then
                ...
                if mount -o ro "$part_dev" "$tmpmnt" 2>/dev/null; then
                    if [[ -d "$tmpmnt/EFI" ]]; then
                        log_info "      [EFI System Partition — UEFI boot]" | tee -a "$IMAGE_INSPECT_LOG"
                        IMAGE_BOOTLOADER="uefi"
                        ls -la "$tmpmnt/EFI/" >> "$IMAGE_INSPECT_LOG" 2>&1
                    fi
                    umount "$tmpmnt" 2>/dev/null
                fi
            fi

            # Mount root partition for deeper inspection
            if [[ "$ptype" =~ ^(ext4|xfs|btrfs|ext3|ext2|ntfs|ntfs3)$ ]] && [[ -z "$(mountpoint -q "$mountdir" 2>/dev/null && echo mounted)" ]]; then
                if mount -o ro "$part_dev" "$mountdir" 2>/dev/null; then
